...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!product) {
    return (
      <div className="py-24 px-6 text-center space-y-4 max-w-md mx-auto bg-[#fffdf9] text-[#252525]">
        <h1 className="font-serif text-3xl font-normal text-[#252525]">Formula Not Found</h1>
        <p className="text-xs font-mono text-[#252525]/60">The requested botanical formula could not be located in our archives.</p>
        <Link href="/shop" className="inline-block bg-[#252525] text-white px-6 py-3 rounded-none text-xs font-mono uppercase tracking-widest">
          Explore Catalog →
        </Link>
      </div>
    );
  ...
  const [openAccordion, setOpenAccordion] = useState<'ingredients' | 'howTo' | 'shipping' | null>('ingredients');
  ...
  return (
    <div className="py-4 sm:py-8 px-4 sm:px-8 lg:px-12 max-w-[1400px] mx-auto space-y-16 pb-28 lg:pb-16 bg-[#fffdf9] text-[#252525]">
      ...
      <div className="flex items-center gap-1.5 text-[11px] font-mono text-[#b87760] uppercase tracking-wider">
        <Link href="/" className="hover:text-[#252525] transition-colors">Maison</Link>
        <ChevronRight className="w-3 h-3 text-[#252525]/40" />
        <Link href="/shop" className="hover:text-[#252525] transition-colors">Catalog</Link>
        <ChevronRight className="w-3 h-3 text-[#252525]/40" />
        <Link href={`/shop?category=${encodeURIComponent(product.category)}`} className="hover:text-[#252525] transition-colors">{product.category}</Link>
        <ChevronRight className="w-3 h-3 text-[#252525]/40" />
        <span className="text-[#252525] font-bold truncate max-w-[180px] sm:max-w-none">{product.name}</span>
      </div>
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs font-mono text-[#b87760]">
              ...
              <span className="text-[#252525]/60 font-medium">{product.collection}</span>
            </div>
            ...
            <p className="text-xs font-mono text-[#b87760] font-bold tracking-wide uppercase">{product.subtitle}</p>
          </div>
          ...
          <div className="flex items-center justify-between py-3 border-y border-[#252525]/15">
            <div className="flex items-center gap-2 text-xs font-mono">
              <div className="flex text-[#252525]">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className="w-4 h-4 fill-current text-[#252525]" />
                ))}
              </div>
              <span className="font-bold text-[#252525] text-sm">{product.rating}</span>
              <span className="text-[#252525]/60 text-[11px]">({product.reviewCount} verified reviews)</span>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2.5 text-xs font-sans text-[#252525]">
              {product.benefits.map((b, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-[#f5f3ed] p-2.5 border border-[#252525]/10">
                  <CheckCircle2 className="w-4 h-4 text-[#b87760] flex-shrink-0" />
                  <span className="truncate text-xs text-[#252525]/90 font-medium">{b}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center gap-3">
              ...
              >
                {added ? (
                  ...
                  <>
                    <ShoppingBag className="w-4 h-4 text-[#b87760]" /> Add to cart — {formatMad(product.price)}
                  </>
                )}
              </button>
              ...
              <button
                ...
                className={`p-4 rounded-none border border-[#252525]/20 transition-colors ${
                  isSaved ? 'bg-[#b87760]/20 text-[#b87760] border-[#b87760]' : 'hover:bg-[#252525]/5 text-[#252525]'
                }`}
            ...
            >
              Instant Checkout with Cash on Delivery <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          ...
          <div className="pt-4 space-y-2 border-t border-[#252525]/15">
            ...
            <div className="border border-[#252525]/15 bg-[#fffdf9]">
              ...
              {openAccordion === 'ingredients' && (
                <div className="p-4 bg-[#f5f3ed] border-t border-[#252525]/10 space-y-2 text-xs font-sans text-[#252525]/85">
                  <p className="italic text-[#252525]/70 text-xs">Sensory Texture: {product.texture}</p>
            ...
            <div className="border border-[#252525]/15 bg-[#fffdf9]">
              <button
                onClick={() => setOpenAccordion(openAccordion === 'howTo' ? null : 'howTo')}
                className="w-full p-3.5 flex items-center justify-between text-left text-xs font-mono text-[#252525] font-bold hover:bg-[#f5f3ed] transition-colors"
              >
                <div className="flex items-center gap-2">
            ...
            <div className="border border-[#252525]/15 bg-[#fffdf9]">
              ...
              {openAccordion === 'shipping' && (
                <div className="p-4 bg-[#f5f3ed] border-t border-[#252525]/10 space-y-1.5 text-xs font-mono text-[#252525]">
                  <p>🚚 Morocco Express Shipping: Delivered within 24 to 48 hours in all major cities.</p>
                  <p>💵 Paiement à la livraison (Cash on Delivery) available nationwide.</p>
      ...
      <div className="space-y-8 max-w-4xl mx-auto py-8">
        ...
        <div className="grid sm:grid-cols-3 gap-6 pt-4">
          {product.ingredients.slice(0, 3).map((ing, i) => (
            <div key={i} className="bg-[#f5f3ed] p-5 border border-[#252525]/15 space-y-2 text-center">
              ...
              <h3 className="font-serif text-sm font-bold text-[#252525]">{ing}</h3>
              <p className="text-[11px] font-sans text-[#252525]/70">
                Ethically harvested at peak potency to lock in active antioxidants and essential lipids.
              </p>
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#252525] text-white p-8 sm:p-12 space-y-8 shadow-xl">
        ...
        <div className="grid md:grid-cols-3 gap-6 text-xs font-sans">
          <div className="bg-[#1c1c1c] p-6 border border-white/10 space-y-3">
            <span className="text-xs font-mono text-[#b87760] font-bold block">STEP 01 / PREPARE</span>
            <h4 className="font-serif text-base font-normal text-white">Cleanse & Tone</h4>
            <p className="text-white/70 leading-relaxed text-[11px]">
              Start with thoroughly cleansed skin. Mist with Damask Rose Cloud Essence to prepare skin porosity.
            </p>
          </div>

          <div className="bg-[#1c1c1c] p-6 border border-white/10 space-y-3">
            <span className="text-xs font-mono text-[#b87760] font-bold block">STEP 02 / DISPENSE</span>
            <h4 className="font-serif text-base font-normal text-white">Warm & Activate</h4>
            <p className="text-white/70 leading-relaxed text-[11px]">
              {product.howTo}
            </p>
          </div>
          ...
          <div className="bg-[#1c1c1c] p-6 border border-white/10 space-y-3">
            <span className="text-xs font-mono text-[#b87760] font-bold block">STEP 03 / SEAL</span>
            <h4 className="font-serif text-base font-normal text-white">Lock Hydration</h4>
            <p className="text-white/70 leading-relaxed text-[11px]">
              Follow with your favorite cream or moisturizer. Breathe in the subtle natural botanical notes.
            </p>
          </div>
        </div>
      </div>
      ...
      {relatedProducts.length > 0 && (
        <div className="pt-8 border-t border-[#252525]/10 space-y-8">
          <div className="flex justify-between items-end">
            ...
            <Link href="/shop" className="text-xs font-mono uppercase tracking-widest text-[#252525] hover:text-[#b87760] font-bold">
            ...
            {relatedProducts.map((item) => (
              ...
              >
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-mono uppercase text-[#b87760] block font-bold">{item.category}</span>
                  <Link
                  ...
                  <span className="font-serif text-sm font-bold text-[#252525] block">{formatMad(item.price)}</span>
                </div>
                ...
                >
                  <ShoppingBag className="w-3.5 h-3.5 text-[#b87760]" /> Quick Add
                </button>
              </div>
            ))}
          </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 bg-[#f5f3ed] p-6 border border-[#252525]/15 space-y-4">
            ...
            <div className="flex items-baseline gap-3">
              <span className="font-serif text-5xl font-normal text-[#252525]">{product.rating}</span>
              <div className="space-y-0.5">
                ...
                <span className="text-xs font-mono text-[#252525]/60">based on {product.reviewCount} reviews</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-8 space-y-4">
            <h3 className="font-serif text-2xl font-normal text-[#252525]">Customer Testimonials</h3>
            
            <div className="space-y-4">
              {reviews.map((rev) => (
                <div key={rev.id} className="bg-[#fffdf9] p-5 border border-[#252525]/15 space-y-2">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-serif text-base font-normal text-[#252525]">{rev.author}</span>
                        <span className="text-[10px] font-mono bg-[#252525] text-white px-2 py-0.5 uppercase tracking-wider font-bold">
                          Verified Buyer
                        </span>
                      </div>
                      <span className="text-[11px] font-mono text-[#252525]/50">{rev.location} · {rev.date}</span>
                    </div>

                    <div className="flex text-[#252525]">
                      {[...Array(rev.rating)].map((_, i) => (
                        <Star key={i} className="w-3.5 h-3.5 fill-current text-[#252525]" />
                      ))}
                    </div>
                  </div>

                  <h4 className="font-serif text-sm font-bold text-[#252525]">{rev.title}</h4>
                  <p className="text-xs font-sans text-[#252525]/85 leading-relaxed">{rev.comment}</p>
                  ...
                  <div className="flex items-center gap-2 pt-2 text-[11px] font-mono text-[#252525]/60">
                    <button className="flex items-center gap-1 hover:text-[#252525] transition-colors">
                      <ThumbsUp className="w-3 h-3 text-[#b87760]" /> Helpful ({rev.helpfulCount})
                    </button>
                  </div>
                </div>
              ))}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#252525] text-white p-3 px-4 shadow-2xl border-t border-white/20 flex items-center justify-between gap-3 animate-in slide-in-from-bottom duration-300">
        <div className="min-w-0">
          <h4 className="font-serif text-sm font-normal text-white truncate">{product.name}</h4>
          <span className="font-serif text-xs font-bold text-[#b87760]">{formatMad(product.price)}</span>
        </div>
